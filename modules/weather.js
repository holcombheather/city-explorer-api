...
async function getWeather(request, response, next) {
  try {
    ...
    const url = `http://api.weatherbit.io/v2.0/forecast/daily?key=${process.env.REACT_APP_WEATHERBIT_API_KEY}&lat=${lat}&lon=${lon}`;

    const foundCityWeather = await axios.get(url);

    const forecasts = foundCityWeather.data.data.map((obj) => new Forecast(obj));
